            ...
            <div className="bg-white dark:bg-[#0b0914]/80 border border-purple-100 dark:border-purple-900/40 rounded-3xl overflow-hidden shadow-md dark:shadow-2xl flex flex-col transition-colors duration-300">
              ...
              <div className="p-5 flex-1">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-3 gap-2">
                  <span className="text-xs font-bold text-amber-700 dark:text-amber-300">Itemized Employee Payslips</span>
                  <span className="text-xs text-slate-600 dark:text-purple-300/70 font-mono">
                    Total Gross: ${selectedPayrun.payslips?.reduce((s: number, p: any) => s + p.grossTotal, 0).toLocaleString('en-US', { minimumFractionDigits: 2 })} | Total Net: <strong className="text-amber-600 dark:text-amber-300">${selectedPayrun.payslips?.reduce((s: number, p: any) => s + p.netTotal, 0).toLocaleString('en-US', { minimumFractionDigits: 2 })}</strong>
                  </span>
                </div>
                    ...
                    <tbody className="divide-y divide-purple-100 dark:divide-purple-950 text-slate-800 dark:text-purple-100 font-medium">
                      {selectedPayrun.payslips?.map((p: any) => (
                        <tr key={p.id} className="hover:bg-purple-50/50 dark:hover:bg-purple-950/20 transition">
                          ...
                          <td className="px-4 py-3.5 font-mono text-purple-700 dark:text-purple-300">{p.workedDays} days</td>
                          <td className="px-4 py-3.5 font-mono font-semibold text-slate-700 dark:text-purple-200">${p.grossTotal.toLocaleString('en-US', { minimumFractionDigits: 2 })}</td>
                          <td className="px-4 py-3.5 font-mono font-black text-amber-600 dark:text-amber-300 text-sm">
                    ...
                    {previewResult.eligibleEmployees.map((emp: any) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="text-[10px] font-mono text-amber-600 dark:text-amber-300 font-bold">${emp.wage.toLocaleString()}/mo</span>
                        </label>
                      );
                    })}
                  </div>
          ...
          <div className="bg-white dark:bg-[#090712] border border-purple-200 dark:border-purple-800/60 rounded-3xl w-full max-w-lg p-6 shadow-2xl space-y-4 transition-colors duration-300">
              ...
              {selectedPayslip.lines?.map((l: any) => {
                ...
                return (
                  <div key={l.id} className="flex items-center justify-between p-2.5 rounded-xl bg-slate-50 dark:bg-[#06050b] border border-slate-200 dark:border-purple-900/40 text-xs">
                    ...
                    <span className={`font-mono font-black ${isDeduction ? 'text-rose-600 dark:text-rose-400' : 'text-amber-600 dark:text-amber-300'}`}>
            ...
            <div className="p-3.5 bg-slate-50 dark:bg-[#06050b] border border-slate-200 dark:border-purple-900/50 rounded-2xl space-y-1 text-xs">
              <div className="flex justify-between text-slate-600 dark:text-purple-300/70 font-medium">
                ...
                <span className="font-mono font-bold text-slate-900 dark:text-white">${selectedPayslip.grossTotal.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
              </div>
