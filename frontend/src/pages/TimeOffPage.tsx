    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-xs text-purple-400/60 font-mono">{allocations.length} Active Grants</span>
        </div>

        {allocations.length === 0 ? (
          <div className="p-8 text-center bg-[#0b0914]/60 border border-purple-900/30 rounded-3xl text-purple-300/60 text-xs">
            No leave allocations configured. Click <strong>"Grant Leave Allocation"</strong> above to assign quotas.
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {allocations.map((alloc) => (
              <div
                key={alloc.id}
                className="bg-[#0b0914]/80 border border-purple-900/40 hover:border-amber-400/60 p-5 rounded-3xl relative overflow-hidden shadow-xl transition-all duration-300 hover:scale-[1.01] group"
              >
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    {canManage && (
                      <button
                        ...
                        className="opacity-0 group-hover:opacity-100 p-1 rounded-lg text-rose-400/70 hover:text-rose-400 hover:bg-rose-500/10 transition"
                ...
                <div className="my-3">
                  ...
                  <div className="text-xs text-purple-300/70 mt-1 flex items-center gap-1">
                    ...
                    Employee: <strong className="text-white ml-1">{alloc.employee?.name}</strong>
                  </div>
                  <div className="text-[10px] text-purple-400/60 mt-0.5 font-mono">
                    Valid: {new Date(alloc.validFrom).toISOString().slice(0, 10)} → {new Date(alloc.validTo).toISOString().slice(0, 10)}
                  </div>
                </div>
      ...
      <div className="bg-[#0b0914]/80 border border-purple-900/40 rounded-3xl overflow-hidden shadow-2xl">
        ...
        <table className="w-full text-left text-xs">
          ...
          <tbody className="divide-y divide-purple-950 text-purple-100">
            {requests.length === 0 ? (
              <tr>
                <td colSpan={7} className="px-5 py-8 text-center text-purple-400/50">
                    ...
                    <div className="flex items-center justify-end gap-2">
                      ...
                      {(canManage || req.status === 'Pending') && (
                        <button
                          ...
                          className="p-1.5 rounded-lg text-purple-400/60 hover:text-rose-400 hover:bg-rose-500/10 transition"
